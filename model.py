...
class Model(nn.Module):
    def __init__(self, config, bert_config):
        ...
        self.Cr = nn.Linear(config.cr* len(config.dilation), config.label_num*config.label_num)
        ...
        self.output_old=nn.Linear(config.old_label_num, config.cr* len(config.dilation))
        torch.nn.init.orthogonal_(self.output_old.weight, gain=1)
        self.output_new=nn.Linear(config.new_label_num, config.cr* len(config.dilation))
